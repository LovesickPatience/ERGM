...
from src.model.qwen2_5_omni_with_fusion import *
...
class Manager:
        ...
        for epoch in range(start_epoch, start_epoch + self.args.num_epochs):
            ...
            if valid_ppl < self.best_ppl:
                ...
                save_path = os.path.join(
                    self.args.ckpt_dir,
                    f"best_ckpt_epoch={epoch}_valid_ppl={self.best_ppl:.4f}_val_emo_acc={valid_acc:.2f}%_{now}+selector.ckpt",
                )
                if epoch % 2 == 0:
    ...
    def _pool_text_feat(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            emb = self.model.get_input_embeddings()(input_ids)  # (B, L, E)
