...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        # 模态加权融合：仅当提供 imgs/auds 时执行
        # 仅在没有使用 past_key_values（即首次整体前向/解码第 1 步）时进行
        if past_key_values is None and imgs is not None and auds is not None:
            gate_w = gated_weights
            if gate_w is not None and gate_w.dim() == 1:
                gate_w = gate_w.unsqueeze(0)

            auds = auds.to(device=device, dtype=inputs_embeds.dtype)
            imgs = imgs.to(device=device, dtype=inputs_embeds.dtype)
            if auds.dim() == 2:
                auds = auds.unsqueeze(1)
            if imgs.dim() == 2:
                imgs = imgs.unsqueeze(1)

            if gate_w is not None:
                w_t = gate_w[:, 0].view(B, 1, 1).to(device=device, dtype=inputs_embeds.dtype)
                w_a = gate_w[:, 1].view(B, 1, 1).to(device=device, dtype=inputs_embeds.dtype)
                w_v = gate_w[:, 2].view(B, 1, 1).to(device=device, dtype=inputs_embeds.dtype)
                inputs_embeds = inputs_embeds * w_t
                auds = auds * w_a
                imgs = imgs * w_v

            # 拼接：[auds, imgs, text]
            inputs_embeds = torch.cat([auds, imgs, inputs_embeds], dim=1)
            modal_len = auds.size(1) + imgs.size(1)

            # 同步 pad mask、token_type_ids，先补长度，再统一截断到 max_position_embeddings
            if attn_mask_raw is not None:
                attn_mask_raw = torch.cat(
                    [torch.ones(B, modal_len, device=device, dtype=attn_mask_raw.dtype), attn_mask_raw.view(B, -1)],
                    dim=1,
                )
            else:
            ...
            if token_type_ids is not None:
                token_type_ids = torch.cat(
                    [torch.zeros(B, modal_len, device=device, dtype=token_type_ids.dtype), token_type_ids.view(B, -1)],
                    dim=1,
                )
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs):
        ...
        gated_weights = kwargs.get("gated_weights", None)
        if past_key_values:
