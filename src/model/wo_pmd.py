...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        # 模态融合：仅当提供 imgs/auds 时执行（始终以文本为主模态）
        ...
        if past_key_values is None and imgs is not None and auds is not None:
            ...
            text_sel = torch.ones(B, dtype=torch.bool, device=device)
            txt_mask_sel = txt_pad_mask if txt_pad_mask is not None else None
            txt_sel = txt
            aud_sel = auds
            img_sel = imgs
            ...
            cross_a, _ = self.mha_cross(txt_sel, aud_sel, aud_sel, key_padding_mask=aud_kmask)
            cross_v, _ = self.mha_cross(txt_sel, img_sel, img_sel, key_padding_mask=img_kmask)
