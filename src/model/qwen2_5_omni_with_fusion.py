...
def _ensure_rope_parameters(cfg):
    ...
    This project’s attention and rotary embedding code expects:
      cfg.rope_parameters = {
        "rope_type": str,
        "rope_theta": float,
        "factor": float,
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        # 轻量模态融合：在文本侧注入三个伪 token（main_self / cross_1 / cross_2）
    ...
    def forward(
        ...
        # 2. 模态融合：使用三个伪 token 注入 main_self / cross_1 / cross_2，摒弃占位符替换
        gate_w = gated_weights if gated_weights is not None else None
        if gate_w is not None and gate_w.dim() == 1:
            gate_w = gate_w.unsqueeze(0)

        use_fusion = past_key_values is None and imgs is not None and auds is not None and main_idx is not None
        if use_fusion:
            B, L, H = inputs_embeds.shape
            device = inputs_embeds.device
            main_idx = main_idx.to(device)
            if gate_w is not None:
                gate_w = gate_w.to(device)

            def _prep_modal(x, proj):
                if isinstance(x, (list, tuple)):
                    x = torch.stack([self._to_tensor_pool_seq(v, device, inputs_embeds.dtype) for v in x], dim=0)
                elif torch.is_tensor(x) and x.dim() == 3:
                    x = x.to(device=device, dtype=inputs_embeds.dtype)
                elif torch.is_tensor(x):
                    x = x.to(device=device, dtype=inputs_embeds.dtype).unsqueeze(1)
                else:
                    x = torch.as_tensor(x, device=device, dtype=inputs_embeds.dtype)
                if x.dim() == 2:
                    x = x.unsqueeze(1)
                return proj(x).to(device=device, dtype=inputs_embeds.dtype)

            txt = inputs_embeds
            txt_pad_mask = attention_mask.view(B, -1) == 0 if attention_mask is not None else None
            auds_feat = _prep_modal(auds, self.auds_proj)
            imgs_feat = _prep_modal(imgs, self.imgs_proj)

            aud_kmask = torch.zeros(auds_feat.size(0), auds_feat.size(1), dtype=torch.bool, device=device)
            img_kmask = torch.zeros(imgs_feat.size(0), imgs_feat.size(1), dtype=torch.bool, device=device)

            fused_tokens = torch.zeros(B, 3, H, device=device, dtype=inputs_embeds.dtype)

            text_sel = (main_idx == 0)
            if torch.any(text_sel):
                txt_sel = txt[text_sel]
                txt_mask_sel = txt_pad_mask[text_sel] if txt_pad_mask is not None else None
                aud_sel = auds_feat[text_sel]
                img_sel = imgs_feat[text_sel]
                ...
                fused_tokens[text_sel, 0, :] = main_self.mean(dim=1)
                fused_tokens[text_sel, 1, :] = cross_a.mean(dim=1)
                fused_tokens[text_sel, 2, :] = cross_v.mean(dim=1)
                if gate_w is not None:
                    gw = gate_w[text_sel]
                    fused_tokens[text_sel, 0, :] *= gw[:, 0:1]
                    fused_tokens[text_sel, 1, :] *= gw[:, 1:2]
                    fused_tokens[text_sel, 2, :] *= gw[:, 2:3]

            aud_sel = (main_idx == 1)
            if torch.any(aud_sel):
                txt_sel = txt[aud_sel]
                txt_mask_sel = txt_pad_mask[aud_sel] if txt_pad_mask is not None else None
                aud_main = auds_feat[aud_sel]
                img_sel = imgs_feat[aud_sel]
                ...
                fused_tokens[aud_sel, 0, :] = main_self.mean(dim=1)
                fused_tokens[aud_sel, 1, :] = cross_txt.mean(dim=1)
                fused_tokens[aud_sel, 2, :] = cross_img.mean(dim=1)
                if gate_w is not None:
                    gw = gate_w[aud_sel]
                    fused_tokens[aud_sel, 0, :] *= gw[:, 0:1]
                    fused_tokens[aud_sel, 1, :] *= gw[:, 1:2]
                    fused_tokens[aud_sel, 2, :] *= gw[:, 2:3]

            vid_sel = (main_idx == 2)
            if torch.any(vid_sel):
                txt_sel = txt[vid_sel]
                txt_mask_sel = txt_pad_mask[vid_sel] if txt_pad_mask is not None else None
                aud_sel_v = auds_feat[vid_sel]
                img_main = imgs_feat[vid_sel]
                ...
                fused_tokens[vid_sel, 0, :] = main_self.mean(dim=1)
                fused_tokens[vid_sel, 1, :] = cross_txt.mean(dim=1)
                fused_tokens[vid_sel, 2, :] = cross_aud.mean(dim=1)
                if gate_w is not None:
                    gw = gate_w[vid_sel]
                    fused_tokens[vid_sel, 0, :] *= gw[:, 0:1]
                    fused_tokens[vid_sel, 1, :] *= gw[:, 1:2]
                    fused_tokens[vid_sel, 2, :] *= gw[:, 2:3]

            inputs_embeds = torch.cat([fused_tokens, inputs_embeds], dim=1)
            fusion_tokens = fused_tokens.size(1)

            if input_ids is not None:
                pad_id = self.pad_token_id if self.pad_token_id is not None else 0
                prefix_ids = torch.full(
                    (B, fusion_tokens), pad_id, device=input_ids.device, dtype=input_ids.dtype
                )
                input_ids = torch.cat([prefix_ids, input_ids], dim=1)

            if attention_mask is None:
                attention_mask = torch.ones(B, inputs_embeds.size(1), device=device, dtype=torch.long)
            else:
                attention_mask = attention_mask.view(B, -1)
                fusion_mask = torch.ones(B, fusion_tokens, device=device, dtype=attention_mask.dtype)
        ...
        fusion_offset = fusion_tokens if use_fusion else 0
        if fusion_offset > 0:
            hidden_states_text = hidden_states[:, fusion_offset:, :]
        else:
            hidden_states_text = hidden_states

        logits = self.lm_head(hidden_states_text)

        last_token_hidden_state = hidden_states_text[:, -1, :]
